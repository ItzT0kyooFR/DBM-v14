module.exports = {
  ...
  fields: ["role", "varName2", "perms", "varNamePerms", "changeTo", "reason"],
  ...
  async action(cache) {
    ...
    if (Array.isArray(permissions)) {
      for (const perm of permissions) {
        if (changeTo === true) perms.add(perm);
        else if (changeTo === false) perms.remove(perm);
      }
    } else {
    ...
    try {
      await role.setPermissions(perms, reason);
    } catch (err) {
