module.exports = {
  ...
  subtitle(data) {
    return `Searching for a webhook: ${data.webhookId || "No Webhook"}`;
  },
  ...
  fields: ["webhookId", "webhookToken", "storage", "varName"],
...
<div style="float: left; width: 35%;">
  ...
  <input id="webhookId" class="round" type="text">
</div>
<div style="float: right; width: 60%;">
  <span class="dbminputlabel">Webhook Token</span>
  <input id="webhookToken" class="round" type="password">
</div>
<br><br><br><br>
...
`
  ...
  action(cache) {
    ...
    const webhookId = this.evalMessage(data.webhookId, cache);
    const webhookToken = this.evalMessage(data.webhookToken, cache);
    try {
      ...
      this.storeValue(webhook, storage, varName, cache);
    } catch (err) {
