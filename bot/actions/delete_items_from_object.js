module.exports = {
  ...
  subtitle(data, presets) {
    ...
    const count = data.entriess?.length || 0;
    return `Delete ${count} key${count === 1 ? "" : "s"} from ${
...
<br><br><br><br>
<dialog-list id="entriess" fields='["key", "value"]' saveButtonText="Save Item" dialogTitle="Add Entry" dialogWidth="360" dialogHeight="170" listLabel="Entries" listStyle="height: calc(100vh - 285px);" itemName="Entry" itemCols="2" itemHeight="40px;" itemStyle="padding: 10px; text-align: left; " itemTextFunction="data.key ? data.value ? data.key + ': ' + data.value : 'Clear \\'' + data.key + '\\'' : '<a style=\\'color:red\\'>Missing key</a>'">
...
`
  ...
  action(cache) {
    ...
    for (const entry of data.entriess) {
      const key = this.evalMessage(entry.key, cache);
      if (!key) continue;
      delete object[key];
    }
