...
`
  ...
  init() {
    ...
    glob.change = (e) => {
      const div = document.getElementById("serverInputWrapper");
      div.style.opacity = e.value ? "0.5" : "1";
      div.style.pointerEvents = e.value ? "none" : "auto";
      div
        .querySelectorAll("input, select, button, textarea")
        .forEach((el) => (el.disabled = e.value));
    };
  ...
  async action(cache) {
    ...
    let result;
    if (data.mode) {
      emojis = this.getClient().emojis.cache;
    } else {
