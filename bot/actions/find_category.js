...
<div>
  <div style="float: left; width: 40%;">
    ...
    <select id="info" class="round">
      <option value="0" selected>Same Category</option>
...
<store-in-variable style="padding-top: 8px;" dropdownLabel="Store In" selectId="storage" variableContainerId="varNameContainer2" variableInputId="varName2"></store-in-variable>`
  ...
  init() {
    ...
    const findInputContainer = document.querySelector("#find").parentElement;
    const toggleFindInput = () => {
      if (infoSelect.value === "0") {
    ...
    toggleFindInput();
    infoSelect.addEventListener("change", toggleFindInput);
  },
  ...
  async action(cache) {
    ...
    switch (info) {
      case 0:
        // same category
        {
          const tempChannel = cache.msg || cache.interaction?.channel;
          result = tempChannel?.parent || null;
        }
        break;
      case 1:
        // category id
        result = categories.get(find);
        break;
      case 2:
        // category name
        result = categories.find((c) => c.name === find);
        break;
      case 3:
        // category position
        const pos = parseInt(find, 10);
        result = categories.find((c) => c.position === pos);
        break;
      default:
        break;
    }
