module.exports = {
  ...
  subtitle(data, presets) {
    return `Create Thread Named "${data.threadName}"`;
  },
...
<div style="float: left; width: 100%;">
  ...
  <input id="threadName" class="round" type="text">
</div>
...
<input id="threadReason" class="round" type="text" placeholder="Leave blank for none...">
...
<store-in-variable allowNone selectId="storage" variableInputId="storageVarName" variableContainerId="varNameContainer2"></store-in-variable>`
  ...
  async action(cache) {
    ...
    const payload = {
      ...
      reason: this.evalMessage(data.threadReason, cache) || undefined,
    };

    try {
      const threadChannel = await message.startThread(payload);
