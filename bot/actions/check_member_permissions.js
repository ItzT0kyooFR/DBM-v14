module.exports = {
  ...
  fields: ["member", "varName", "perms", "varNamePerms", "branch"],
...
<conditional-input id="branch"></conditional-input>`
  ...
  async action(cache) {
    ...
    let result = false;
    if (member) {
      if (!member.permissions && server) {
        member = await server.members.fetch(member.id).catch(() => null);
      }
      if (member && member.permissions) {
        result = permissions.every((perm) => member.permissions.has(perm));
      }
    }
