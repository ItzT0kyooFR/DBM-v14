module.exports = {
  ...
  fields: ["options", "wordLength", "customCharacters", "storage", "varName"],
...
<div style="float: left; width: 60%;">
  <span class="dbminputlabel">Randomize Letters</span>
	<select id="options" class="round">
		<option value="a">abcdefghijklmnopqrstuvwxyz</option>
    <option value="A">ABCDEFGHIJKLMNOPQRSTUVWXYZ</option>
		<option value="0">0123456789</option>
    <option value="!">~!@#$%^&()_+-={}[];\\',.</option>
    <option value="*">All Of The Above Combined</option>
    <option value="?">Custom Characters</option>
	</select>
</div>
...
<br><br><br>
<div id="customCharactersWrapper">
  <span class="dbminputlabel">Custom Characters</span>
  <input id="customCharacters" class="round" type="text">
</div>
<br>
...
`
  ...
  init() {
    ...
    const customWrapper = document.getElementById("customCharactersWrapper");
    ...
    optionsSelect.addEventListener("change", updateVisibility);
    updateVisibility();
  },
  ...
  async action(cache) {
    ...
    const customCharacters = this.evalMessage(data.customCharacters, cache);
    function randomizeLetters(opt, len, custom = "") {
      const sets = {
      ...
      let chars = "";
      for (let o of opt) {
        if (o == "*") chars += sets.a + sets.A + sets["0"] + sets["!"];
        else if (o == "?") chars += custom;
        else if (sets[o]) chars += sets[o];
      }
      let r = "";
      for (let i = 0; i < len; i++)
    ...
    const result = randomizeLetters(options, wordLength, customCharacters);
    this.storeValue(
      result,
