module.exports = {
  ...
  subtitle(data, presets) {
    if (Math.abs(parseInt(data.increment, 10)) === 1) {
      return `Call ${data.actions?.length ?? 0} actions ${
        Math.abs(data.endNum - data.startNum) + 1
      } times.`;
    }
    return `Call ${data.actions?.length ?? 0} actions while counting by ${
      data.increment
    }, from ${data.startNum} to ${data.endNum}.`;
  },
