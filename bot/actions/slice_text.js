module.exports = {
  ...
  subtitle(data) {
    const start = data.startingNumber || 0;
    const length = data.sliceLength || "all";
    return `Slices ${length} character(s) from position ${start}`;
  },
  ...
  html() {
    ...
    return (
      mod +
      `
<span class="dbminputlabel">Text to Slice</span>
<textarea id="textToSlice" rows="3" placeholder="Insert text here..." style="white-space: nowrap; resize: none;"></textarea>
<br>
<div style="float: left; width: 49%;">
  <span class="dbminputlabel">Slice Starting Number</span>
  <input id="startingNumber" class="round" type="text">
</div>
<div style="float: right; width: 49%;">
  <span class="dbminputlabel">Slice Length</span>
...
`
  ...
  action(cache) {
    ...
    const textToSlice = this.evalMessage(data.textToSlice, cache);
    const startingFrom =
      parseInt(this.evalMessage(data.startingNumber, cache), 10) || 0;
    const sliceLengthRaw = this.evalMessage(data.sliceLength, cache);
    let result;
    if (sliceLengthRaw === "" || sliceLengthRaw == null) {
      result = `${textToSlice}`.slice(startingFrom);
    } else {
      const sliceLength = parseInt(sliceLengthRaw, 10);
      result = `${textToSlice}`.slice(startingFrom, startingFrom + sliceLength);
    }
    this.storeValue(
      result,
