module.exports = {
  ...
  subtitle(data, presets) {
    return `Create Post Named "${data.postName}"`;
  },
  ...
  variableStorage(data, varType) {
    ...
    return [data.storageVarName, "Post Channel"];
  },
  ...
  html(isEvent, data) {
    ...
    return (
      mod +
      `
<channel-input dropdownLabel="Source Channel" selectId="channel" variableContainerId="varNameContainer" variableInputId="channelVarName"></channel-input>

<br><br><br><br>

<div style="float: left; width: 100%;">
  <span class="dbminputlabel">Post Name</span><br>
  <input id="postName" class="round" type="text">
</div>

<br><br><br>

<message-input dropdownLabel="Message to Send" selectId="storage1" variableContainerId="varNameContainer1" variableInputId="varName1"></message-input>
...
<input id="postReason" class="round" type="text" placeholder="Leave blank for none...">
...
<store-in-variable allowNone selectId="storage" variableInputId="storageVarName" variableContainerId="varNameContainer2"></store-in-variable>`
  ...
  async action(cache) {
    ...
    const payload = {
      name: this.evalMessage(data.postName, cache),
      message: msg,
      autoArchiveDuration:
        autoArchiveMap[parseInt(data.autoArchiveDuration, 10) || 0],
      rateLimitPerUser:
        parseInt(this.evalMessage(data.slowMode, cache), 10) || 0,
      reason: this.evalMessage(data.postReason, cache) || undefined,
    };

    try {
      const postChannel = await channel.threads.create(payload);
      const storage = parseInt(data.storage, 10);
      const storageVarName = this.evalMessage(data.storageVarName, cache);
      this.storeValue(postChannel, storage, storageVarName, cache);
