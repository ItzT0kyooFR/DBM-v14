module.exports = {
  ...
  fields: ["textToReplace", "text1", "text2", "type", "storage", "varName"],
  ...
  html() {
    ...
    return (
      mod +
      `
<span class="dbminputlabel">Text to Replace</span>
<textarea id="textToReplace" rows="3" placeholder="Insert text here..." style="white-space: nowrap; resize: none;"></textarea>
<br>
...
<div style="width: 40%;">
  ...
  <select id="type" class="round">
    ...
    <option value="1">Replace The Last Result</option>
    <option value="2">Replace All Results</option>
  </select>
</div>
<br><br>
...
`
  ...
  action(cache) {
    ...
    const text = this.evalMessage(data.textToReplace, cache);
    const oldVal = this.evalMessage(data.text1, cache);
    const newVal = this.evalMessage(data.text2, cache);
    const type = parseInt(data.type, 10);
    let result;
    if (type === 0) {
      result = text.replace(oldVal, newVal);
    } else if (type === 1) {
      const index = text.lastIndexOf(oldVal);
      if (index !== -1) {
        result =
          text.slice(0, index) + newVal + text.slice(index + oldVal.length);
      } else {
        result = text;
      }
    } else {
      result = text.split(oldVal).join(newVal);
    }
    if (result !== undefined) {
      this.storeValue(
        result,
