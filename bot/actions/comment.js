module.exports = {
  ...
  subtitle(data) {
    ...
    const decorations = [];
    if (data.underline) decorations.push("underline");
    if (data.overline) decorations.push("overline");
    if (data.strikethrough) decorations.push("line-through");
    if (decorations.length > 0)
      styles += `text-decoration: ${decorations.join(" ")}; `;
    if (data.shadow) styles += "text-shadow: 1px 1px 2px #000; ";
    if (data.glow) styles += "text-shadow: 0 0 5px #0ff, 0 0 10px #0ff; ";
    if (data.code)
      styles +=
        "font-family: monospace; background: rgba(16, 16, 16, 0.3); border-radius: 3px; padding: 2px 4px; ";
  ...
  html() {
    ...
    return (
      mod +
      `
<span class="dbminputlabel">Comment To Show</span>
...
<br>
<span class="dbminputlabel">Other Styles</span>
<dbm-container style="transform: scale(1); transform-origin: top left; width: 100%; display: flex; flex-wrap: wrap; gap: 5px; max-height: 120px; overflow-y: auto; border-radius: 5px;">
    <dbm-checkbox id="bold" label="Bold"></dbm-checkbox>
    <dbm-checkbox id="italic" label="Italic"></dbm-checkbox>
    <dbm-checkbox id="underline" label="Underline"></dbm-checkbox>
    <dbm-checkbox id="overline" label="Overline"></dbm-checkbox>
    <dbm-checkbox id="shadow" label="Shadow"></dbm-checkbox>
    <dbm-checkbox id="strikethrough" label="Strikethrough"></dbm-checkbox>
    <dbm-checkbox id="code" label="Code"></dbm-checkbox>
    <dbm-checkbox id="glow" label="Glow"></dbm-checkbox>
</dbm-container>
`
    );
  },
