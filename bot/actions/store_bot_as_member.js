  ...
  async action(cache) {
    ...
    try {
      if (server.members.me) {
        botMember = server.members.me;
      } else {
        botMember = await server.members.fetch(this.getClientUser().id);
        server.members.me = botMember;
      }
    } catch (err) {
    ...
    this.storeValue(botMember, storage, varName2, cache);
