    ...
    cronScheduler.setupCrons = function setupCrons() {
      ...
      for (const event of Bot.$evts["Cron Scheduler"]) {
        try {
          ...
          const timeZone =
            event.temp2 || Intl.DateTimeFormat().resolvedOptions().timeZone;

          if (!cron.validate(cronString))
            return console.error(
              `[Cron Scheduler] Invalid cron string for '${eventName}': '${cronString}'`
            );
          if (!cronScheduler.isValidTimeZone(timeZone))
            return console.error(
              `[Cron Scheduler] Invalid Timezone for '${eventName}': '${timeZone}'`
            );
          ...
          if (Object.keys(cronScheduler.Jobs))
            console.log(
              `[Cron Scheduler] ${
                Object.keys(cronScheduler.Jobs).length
              } Jobs Scheduled.`
            );
        } catch (error) {
