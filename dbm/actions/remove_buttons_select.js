module.exports = {
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/remove_buttons_select.js",
  },
  ...
  async action(cache) {
    ...
    if (type === "all") {
      ...
      for (let i = 0; i < oldComponents.length; i++) {
        ...
        const comps =
          compData instanceof ActionRowBuilder ? compData.toJSON() : compData;
        const newComps = [];
        const BUTTON_TYPE = 2;
        ...
        for (let j = 0; j < comps.components.length; j++) {
          ...
          switch (type) {
            case "allButtons": {
              if (comp.type !== BUTTON_TYPE || comp.type === "BUTTON")
                newComps.push(comp);
              else deleted = true;
              break;
            }
            case "allSelects": {
              if (!SELECT_TYPES.has(comp.type) || comp.type === "SELECT_MENU")
                newComps.push(comp);
              else deleted = true;
              break;
            }
    ...
    try {
      if (
        ...
        await cache.interaction.update({ components });
