module.exports = {
  ...
  subtitle(data) {
    const info = [
      ...
      "Channel Shorts Count",
      "Channel Live Count",
      "Channel Playlists Count",
      "Channel Posts Count",
      "Channel Shelves",
    ];
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/store_youtube_channel_info.js",
  },
  ...
  html() {
    ...
    return (
      mod +
      `
<span class="dbminputlabel">Channel URL/ID</span>
...
<select id="info" class="round">
  ...
  <option value="3">Channel Handle</option>
  <option value="4">Channel Description</option>
  <option value="5">Channel Thumbnail URL</option>
  <option value="6">Channel Banner URL</option>
  <option value="7">Channel Verified?</option>
  <option value="8">Channel Subscriber Count</option>
  <option value="9">Channel Video Count</option>
  <option value="10">Channel Videos Count</option>
  <option value="11">Channel Shorts Count</option>
  <option value="12">Channel Live Count</option>
  <option value="13">Channel Playlists Count</option>
  <option value="14">Channel Posts Count</option>
  <option value="15">Channel Shelves (array)</option>
</select>
<br>
  ...
  async action(cache) {
    ...
    try {
      ...
      switch (info) {
        ...
        case 1:
          // channel url
          result = `https://www.youtube.com/channel/${channel.id}`;
          break;
        case 2:
          // channel name
          result = channel.name;
        ...
        case 4:
          ...
          break;
        case 5:
          // channel thumbnail url (last)
          result =
            channel.thumbnails?.[channel.thumbnails.length - 1]?.url ?? "";
          break;
        case 6:
          // channel banner url (last)
          result = channel.banner?.[channel.banner.length - 1]?.url ?? "";
          break;
        case 7:
          // channel verified?
          result = channel.verified ?? false;
          break;
        case 8:
          // channel subscriber count
        ...
        case 9:
          // channel video count
          result = channel.videoCount ?? "0";
          break;
        case 10:
          // channel videos count
          result = channel.videos?.items?.length ?? 0;
          break;
        case 11:
          // channel shorts count
          result = channel.shorts?.items?.length ?? 0;
          break;
        case 12:
          // channel live count
          result = channel.live?.items?.length ?? 0;
          break;
        case 13:
          // channel playlists count
          result = channel.playlists?.items?.length ?? 0;
          break;
        case 14:
          // channel posts count
          result = channel.posts?.items?.length ?? 0;
          break;
        case 15:
          // channel shelves (array)
          result = channel.shelves ?? [];
          break;
        default:
          result = null;
      }
