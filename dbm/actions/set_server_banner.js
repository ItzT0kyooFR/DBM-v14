module.exports = {
  ...
  subtitle(data, presets) {
    return `${data.banner || "No Image Provided!"}`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/set_server_banner.js",
  },
  ...
  html(isEvent, data) {
    ...
    return (
      mod +
      `
<span class="dbminputlabel">Banner Image</span>
<input id="banner" class="round" type="text" placeholder="Local path / image url / image buffer">
`
    );
  },
  ...
  async action(cache) {
    ...
    const banner = this.evalMessage(data.banner, cache);
    try {
      await server.setBanner(banner);
    } catch (err) {
