module.exports = {
  ...
  subtitle(data, presets) {
    const m = presets.getMessageText(data.storage, data.varName);
    const t = parseInt(data.emoji, 10);
    if (t === 5) return `Remove all Reactions from ${m}`;
    else return `Remove Reaction from ${m}`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/remove_reaction.js",
  },
...
<div style="padding-top: 8px;">
	<div style="float: left; width: 35%;">
      ...
      <option value="5">All Emojis</option>
...
`
  ...
  async action(cache) {
    ...
    try {
      ...
      const reaction =
        (emojiId && message.reactions.cache.get(emojiId)) ||
        (emojiName &&
          message.reactions.cache.find((r) => r.emoji.name === emojiName)) ||
        message.reactions.cache.find((r) => r.emoji.toString() === raw);
