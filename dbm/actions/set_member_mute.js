module.exports = {
  ...
  subtitle(data, presets) {
    return `${presets.getMemberText(data.member, data.varName2)} - ${
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/set_member_mute.js",
  },
  ...
  fields: ["member", "varName2", "mute", "reason"],
...
`
  ...
  async action(cache) {
    ...
    const mute = this.evalMessage(data.mute, cache);
    ...
    try {
      if (member.voice && member.voice.channel) {
        await member.voice.setMute(mute, reason || undefined);
      }
    } catch (error) {}
