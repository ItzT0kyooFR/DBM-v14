module.exports = {
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/edit_thread_channel.js",
  },
...
<br><br><br><br>
<span class="dbminputlabel">Thread Name</span>
<input id="channelName" class="round" type="text">
<br>
<span class="dbminputlabel">SlowMode (in seconds)</span>
<input id="channelSlowmode" class="round" type="text" placeholder="Leave blank for none...">
<br>
<span class="dbminputlabel">Auto Archive Duration</span>
<select id="channelAutoArchiveDuration" class="round">
	<option value="" selected>Don't edit</option>
...
`
  ...
  async action(cache) {
    ...
    const name = this.evalMessage(data.channelName, cache) || undefined;
    const sm = parseInt(this.evalMessage(data.channelSlowmode, cache), 10);
    const slowmode = isNaN(sm) ? undefined : sm;
    const aadRaw = this.evalMessage(data.channelAutoArchiveDuration, cache);
    const aadIndex = parseInt(aadRaw, 10);
    const autoArchiveDuration = isNaN(aadIndex)
      ? undefined
      : autoArchiveMap[aadIndex];
    ...
    try {
      await channel.edit(payload, { reason });
    } catch (error) {
