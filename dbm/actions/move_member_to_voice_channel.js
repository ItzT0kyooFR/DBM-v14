module.exports = {
  ...
  subtitle(data, presets) {
    return `${presets.getMemberText(
      data.member,
      data.varName,
    )} - ${presets.getVoiceChannelText(data.channel, data.varName2)}`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/move_member_to_voice_channel.js",
  },
  ...
  html(isEvent, data) {
    ...
    return (
      mod +
      `
<member-input style="padding-top: 8px;" dropdownLabel="Member" selectId="member" variableContainerId="varNameContainer" variableInputId="varName" selectWidth="45%" variableInputWidth="50%"></member-input>

<br><br><br>

<voice-channel-input style="padding-top: 8px;" dropdownLabel="Move to Voice Channel" selectId="channel" variableContainerId="varNameContainer2" variableInputId="varName2" selectWidth="45%" variableInputWidth="50%"></voice-channel-input>
...
<div style="padding-top: 8px;">
  ...
  async action(cache) {
    ...
    let member = await this.getMemberFromData(data.member, data.varName, cache);
    ...
    if (member && !member.voice) {
      try {
        member = await cache.server.members.fetch(member.id);
      } catch {
