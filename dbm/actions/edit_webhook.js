module.exports = {
  ...
  subtitle(data) {
    return `Editing a webhook: ${data.varName}`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/edit_webhook.js",
  },
  ...
  fields: ["webhookName", "webhookAvatar", "webhook", "varName"],
  ...
  html() {
    ...
    return (
      mod +
      `
<retrieve-from-variable dropdownLabel="Webhook to Edit" selectId="webhook" variableContainerId="varNameContainer" variableInputId="varName"></retrieve-from-variable>
<br><br><br><br>
<span class="dbminputlabel">New Webhook Name</span>
<input id="webhookName" class="round" type="text" placeholder="Leave blank to not edit...">
<br>
<span class="dbminputlabel">New Webhook Avatar (url/path/buffer)</span>
<input id="webhookAvatar" class="round" type="text" placeholder="Leave blank to not edit...">
`
    );
  },
  ...
  async action(cache) {
    ...
    const type = parseInt(data.webhook, 10);
    const varName = this.evalMessage(data.varName, cache);
    let webhook = this.getVariable(type, varName, cache);
    if (!webhook) return this.callNextAction(cache);
    const name = this.evalMessage(data.webhookName, cache) || undefined;
    const avatar = this.evalMessage(data.webhookAvatar, cache) || undefined;
    try {
      webhook = await webhook.edit({ name: name, avatar: avatar });
    } catch (err) {
