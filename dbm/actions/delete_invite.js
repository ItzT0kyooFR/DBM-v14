module.exports = {
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/delete_invite.js",
  },
  ...
  html() {
    ...
    return (
      mod +
      `
<span class="dbminputlabel">Invite to Delete</span>
<input type="text" class="round" id="invite" placeholder="Code or URL | e.g abcdef or discord.gg/abcdef"></input>

<br>

<span class="dbminputlabel">Reason</span>
<input type="text" class="round" id="reason" placeholder="Leave blank for none..."></input>
`
    );
  },
  ...
  async action(cache) {
    ...
    const inviteCode = this.evalMessage(data.invite, cache);
    const reason = this.evalMessage(data.reason, cache) || undefined;
    const client = this.getClient();
    try {
      const invite = await client.fetchInvite(inviteCode);
      if (invite) invite.delete(reason);
    } catch (err) {
