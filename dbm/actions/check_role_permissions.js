module.exports = {
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/check_role_permissions.js",
  },
  ...
  fields: ["role", "varName", "perms", "varNamePerms", "branch"],
...
<conditional-input id="branch"></conditional-input>`
  ...
  async action(cache) {
    ...
    let role = await this.getRoleFromData(data.role, data.varName, cache);
    ...
    let result = false;
    if (role) {
      if (!role.permissions && server) {
        role = await server.roles.fetch(role.id).catch(() => null);
      }
      if (role && role.permissions) {
        result = permissions.every((perm) => role.permissions.has(perm));
      }
    }
