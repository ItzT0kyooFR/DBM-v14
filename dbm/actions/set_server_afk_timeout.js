module.exports = {
  ...
  subtitle(data, presets) {
    return `Timeout: ${data.timeout || "300"}`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/set_server_afk_timeout.js",
  },
...
<div style="display:flex;align-items:center;gap:6px">
    <div style="flex-grow:1">
        <span class="dbminputlabel">Timeout (in seconds)</span>
        <input id="timeout" class="round" type="text" placeholder="Leave blank for 300..." value="300">
        <select id="timeout_select" class="round" style="display:none;" onchange=" document.getElementById('timeout').value = this.value;">
    ...
    <button class="tiny compact ui icon button" style="margin-top:18px" onclick="i=timeout,s=timeout_select,v=i.style.display!='none';i.style.display=v?'none':'';s.style.display=v?'':'none';v&&(s.value=i.value);this.textContent=v?'Text':'Select'">Select</button>
</div>
<br>
...
`
  ...
  async action(cache) {
    ...
    const timeout = parseInt(this.evalMessage(data.timeout, cache), 10) || 300;
    const reason = this.evalMessage(data.reason, cache) || undefined;
    try {
      await server.setAFKTimeout(timeout, reason);
    } catch (err) {
