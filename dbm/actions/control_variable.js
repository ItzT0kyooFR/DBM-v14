module.exports = {
  ...
  subtitle(data, presets) {
    const varName = presets.getVariableText(data.storage, data.varName);
    let changeType = parseInt(data.changeType, 10);
    if (changeType === 0) changeType = "=";
    else if (changeType === 1) changeType = "+";
    else if (changeType === 2) changeType = "-";
    return `${varName} ${changeType} ${data.value}`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/control_variable.js",
  },
...
<div style="padding-top: 8px;">
  ...
  action(cache) {
    ...
    const changeType = parseInt(data.changeType, 10);
    let result;
    let value;
    try {
      value = this.eval(this.evalMessage(data.value, cache), cache);
    } catch (e) {
      this.displayError(data, cache, e);
    }

    try {
      if (changeType === 0) result = value;
      else if (changeType === 1) result = variable + value;
      else if (changeType === 2) result = variable - value;
    } catch (e) {
      this.displayError(data, cache, e);
    }
