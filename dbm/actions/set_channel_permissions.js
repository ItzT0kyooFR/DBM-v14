module.exports = {
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/set_channel_permissions.js",
  },
  ...
  fields: ["storage", "varName", "perms", "varNamePerms", "changeTo", "reason"],
...
<br><br><br><br>
<retrieve-from-variable dropdownLabel="Permission List" selectId="perms" variableContainerId="varNameContainerPerms" variableInputId="varNamePerms"></retrieve-from-variable>
<br><br><br>
<span class="dbminputlabel">Change To</span><br>
<select id="changeTo" class="round">
...
`
  ...
  async action(cache) {
    ...
    let changeTo;
    if (data.changeTo === "0") changeTo = true;
    else if (data.changeTo === "1") changeTo = false;
    else if (data.changeTo === "2") changeTo = null;
    ...
    if (Array.isArray(permissions)) {
      ...
      this.displayError(data, cache, "Permission list is not an array");
      this.callNextAction(cache);
      return;
    }

    try {
      await channel.permissionOverwrites.edit(server.id, options, {
        reason,
      });
    } catch (err) {
