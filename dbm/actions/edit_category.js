module.exports = {
  ...
  subtitle(data, presets) {
    return `Editing Categories "${data.varName}"`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/edit_category.js",
  },
  ...
  fields: ["storage", "varName", "categoryName", "categoryPosition", "reason"],
  ...
  html(isEvent, data) {
    ...
    return (
      mod +
      `
<retrieve-from-variable dropdownLabel="Category to Edit" selectId="storage" variableContainerId="varNameContainer" variableInputId="varName"></retrieve-from-variable>

<br><br><br><br>

<span class="dbminputlabel">Category Name</span>
<input id="categoryName" class="round" type="text">
<br>
<span class="dbminputlabel">Category Position</span>
<input id="categoryPosition" class="round" type="text">
<br>
...
`
  ...
  async action(cache) {
    ...
    const category = this.getVariable(
    ...
    const categoryName =
      this.evalMessage(data.categoryName, cache) || undefined;
    const categoryPosition =
      parseInt(this.evalMessage(data.categoryPosition, cache), 10) || undefined;
    const reason = this.evalMessage(data.reason, cache) || undefined;

    const payload = {};
    if (categoryName) payload.name = categoryName;
    if (!isNaN(categoryPosition)) payload.position = categoryPosition;

    try {
      await category.edit(payload, { reason });
    } catch (err) {
