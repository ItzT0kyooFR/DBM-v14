module.exports = {
  ...
  subtitle(data) {
    return `Find "${data.textToFind}" in "${data.text}"`;
  },
  ...
  meta: {
    ...
    downloadUrl:
      "https://github.com/shadoow051/DBM-v14/blob/main/bot/actions/find_text.js",
  },
  ...
  fields: ["text", "textToFind", "position", "storage", "varName"],
...
<textarea id="text" rows="3" placeholder="Insert text here..." style="white-space: nowrap; resize: none;"></textarea>
<br>
<div style="float: left; width: 69%;">
  <span class="dbminputlabel">Text to Find</span>
  <input id="textToFind" class="round" type="text">
</div>
...
`
  ...
  action(cache) {
    ...
    const textToFind = this.evalMessage(data.textToFind, cache);
    const position = parseInt(data.position, 10);
    let result;
    switch (position) {
      case 0:
        result = text.indexOf(textToFind);
        break;
      case 1:
        result = textToFind.length + text.indexOf(textToFind);
        break;
      default:
        break;
    }
